...
| **交互逻辑** | 原生 **JavaScript** | ① 零构建工具，写完双击就能看<br>② MVP 只有 5 个视图（含门面页），**用框架等于高射炮打蚊子**<br>③ 省掉 1–2 天的工具链配置时间 | 项目变大后，代码要靠自己分文件组织 |
...
| 没选的东西 | 为什么 |
|---|---|
| React / Vue 等前端框架 | 需要构建工具链，配置成本高，与"5 个视图"的规模不匹配 |
...
| 文件 | 干什么 | 为什么单独成一个文件 |
|---|---|---|
| `index.html` | 5 个视图的容器 + 详情弹窗的容器 | 单页应用，所有视图共用一张页面 |
...
| `js/drawer.js` | 只读的详情抽屉：三个只读分页（照片 / 故事 / 时间轴）、Esc 关闭、焦点归还 | 和 `detail.js` 是两套交互（一个只读、一个可写），混在一个文件里会难以取舍 |
| `assets/landing-bg.jpg` | 门面页的背景图 | 静态资源，不参与逻辑 |
...
| 三个视图各自的空状态 | 各视图模块内 | ✅ |
| **5 个视图之间怎么切换** | `js/main.js` + `index.html` 的导航 | ⚠️ **PRD 没有定义导航** → 见第七节 |
...
| 缺口 | 本设计的临时处理 |
|---|---|
| PRD 没定义**导航**（5 个视图怎么切） | 在 `index.html` 顶部加一个导航栏，`js/main.js` 管切换 |
