...
(function (global) {
  ...
  /* 各类型的显示颜色（颜色用来区分"类型"，不表达时长 —— PRD 4.2 #1）
     ⚠️ Day 8 美化改版（最终定稿）：这四个色为【奶油暖色地图底】而选。
        ...
        四者的明度差保持在可辨识范围内。 */
  /* ⚠️⚠️ Day 11 重调：这四个色原来是**为"奶油暖色地图底"选的**，
     但底图早在 Day 8.6 就换成了 `amap://styles/fresh`（草色青，**浅绿**）。
     换成浅绿底之后实测（对底图的对比度）：

       类型        原色        对比度    判定
       长期居住    #2F5D8A     4.96      ✅ 本来就达标
       短期停留    #3D7A5F     3.65      ⚠️ 勉强（而且**绿点画绿底**）
       旅游        #C9761F     2.48      ❌ 连 3:1 都不到
       想去        #9C9287     2.20      ❌ 连 3:1 都不到
     （WCAG 对"有意义的图形"最低要求 3:1；小点要"一眼看到"最好 >= 4.5）
     → 他 2026-09-22 反馈："点的颜色太淡了，没有线的时候不能一下子看到点在哪里"

     改法（**三变一不变**）：
       · 长期居住 藏蓝 —— **不动**。它本来就达标；而且留着它偏亮的明度，
         正好和压深后的「短期停留」拉开**明度差**，两类更容易分开
       · 短期停留 松绿 → **深青**：既提对比，又**偏离绿色相**（绿点画绿底是根因）
       · 旅游 / 想去 —— **压深**：2.48 / 2.20 → 4.41 / 4.88

     ⭐ 改完四色对底图的对比度**全部 >= 4.4:1**（最低要求 3:1，留了余量）
     ⭐ 色相分布：210° 蓝 / 171° 青绿 / 29° 橙 / 34° 灰褐，彼此隔得开
        （「橙」和「灰褐」色相接近，靠**饱和度**区分：一个是饱和的橙，一个是低饱和的灰）
     ⚠️ 别再只改其中一两个：这四个色的**明度梯度**是一起调的，单独动一个会破坏区分度。 */
  var TYPE_COLORS = {
    long:     '#2F5D8A',   /* 长期居住：藏蓝（Day 11 未改；对底图 4.96:1） */
    short:    '#1F6157',   /* 短期停留：深青（原 #3D7A5F 松绿；5.21:1） */
    travel:   '#95500E',   /* 旅游：深暖橙（原 #C9761F；4.41:1） */
    wishlist: '#635A4E'    /* 想去：深暖灰（原 #9C9287；4.88:1） */
  };
